...
pub struct InitializeSnapshot<'info> {
    pub counter: Option<Account<'info, fuzz_example0::Counter>>,
    pub user: Signer<'info>,
    pub system_program: Program<'info, System>,
}
pub struct UpdateSnapshot<'info> {
    pub counter: Account<'info, fuzz_example0::Counter>,
...
impl<'info> InitializeSnapshot<'info> {
    pub fn deserialize_option(
        ...
        let counter: Option<anchor_lang::accounts::account::Account<fuzz_example0::Counter>> =
            accounts_iter
                .next()
                ...
                .map(|acc| {
                    if acc.key() != PROGRAM_ID {
                        anchor_lang::accounts::account::Account::try_from(acc).map_err(|_| {
                            FuzzingError::CannotDeserializeAccount("counter".to_string())
                        })
                    } else {
...
impl<'info> UpdateSnapshot<'info> {
    pub fn deserialize_option(
        ...
        let mut accounts_iter = accounts.iter();
        let counter: anchor_lang::accounts::account::Account<fuzz_example0::Counter> =
            accounts_iter
                .next()
                .ok_or(FuzzingError::NotEnoughAccounts("counter".to_string()))?
                .as_ref()
