...
pub async fn parse_to_idl_program(name: String, code: &str) -> Result<IdlProgram, Error> {
    ...
    let program_id_bytes = {
        let new_pubkey_call = match static_program_id.expr.as_ref() {
        ...
        match new_pubkey_call.args.iter().next() {
    ...
    instruction_item_fns
        ...
        .map(|item_fn| {
            ...
            let account_group_name = item_fn.block.stmts.iter().find_map(|stmt| {
                ...
                let init_expr = *local.init.clone()?.1;
    ...
    let unit_field = syn::punctuated::Punctuated::new();
    for (idl_instruction, _) in &mut instruction_account_pairs {
        ...
        idl_instruction.parameters = instruction_item_struct_fields
            .map(|field| {
                let parameter_name = field.ident.as_ref().unwrap().to_string();
                let parameter_id_type = field.ty.clone().into_token_stream().to_string();
